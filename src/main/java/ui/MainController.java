  ...
  public void showCaseSelectionMenu() {
    Platform.runLater(
        () -> {
          ...
          // Fully tear the case down so entering another case is a clean slate. Previously this SP
          // "return to case selection" path did far less than returnToMainMenu: it never closed the
          // in-game sub-windows/overlays, never reset the exam/tutorial state or the Final-Exam
          // freeze, and never cleared the terminal. That leaked state and accumulated surfaces across
          // case -> selection -> case hops (the lag), and left the previous case's transcript and a
          // half-typed command sitting in the terminal. teardownInGameSurfaces() also resets the
          // Final-Exam cooldown, so no separate call is needed.
          teardownInGameSurfaces();
          gameScreenController.resetCaseContent();
          onInsightTokensUpdate(0); // drop the previous case's token count
          // Clear the terminal completely: the scrollback transcript AND the half-typed command box.
          clearTerminal();
